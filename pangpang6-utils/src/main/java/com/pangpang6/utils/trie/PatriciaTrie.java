package com.pangpang6.utils.trie;
...
public class PatriciaTrie<K, V> extends AbstractMap<K, V> implements Trie<K, V>, Serializable {
        ...
        /**
         * Adds a new <key, value> pair to the Trie and if a pair already exists it
         * will be replaced. In the latter case it will return the old value.
         */
        ...
        /**
         * Returns the Value whose Key equals our lookup Key or null if no such key
         * exists.
         */
        ...
        /**
         * Returns the Value whose Key has the longest prefix in common with our
         * lookup key.
         */
        @SuppressWarnings("unchecked")
        ...
        /**
         * This is equivalent to the other selectR() method but without its overhead
         * because we're selecting only one best matching Entry from the Trie.
         */
        ...
        /**
         * Returns a view of this Trie of all elements that are prefixed by the
         * given key.
         *
         * In a fixed-keysize Trie, this is essentially a 'get' operation.
         *
         * For example, if the trie contains 'Lime', 'LimeWire', 'LimeRadio', 'Lax',
         * 'Later', 'Lake', and 'Lovely', then a lookup of 'Lime' would return
         * 'Lime', 'LimeRadio', and 'LimeWire'.
         *
         * The view that this returns is optimized to have a very efficient
         * Iterator. The firstKey, lastKey & size methods must iterate over all
         * possible values in order to determine the results. This information is
         * cached until the Patricia tree changes. All other methods (except
         * Iterator) must compare the given key to the prefix to ensure that it is
         * within the range of the view. The Iterator's remove method must also
         * relocate the subtree that contains the prefixes if the entry holding the
         ...
         * length of the key.
         *
         * Fixed-keysize Tries will not support this operation (because all keys
         * will be the same length).
         *
         * For example, if the trie contains 'Lime', 'LimeWire', 'LimeRadio', 'Lax',
         * 'Later', 'Lake', and 'Lovely', then a lookup of 'LimePlastics' with a
        ...
        /**
         * Returns a view of this Trie of all elements that are prefixed by the key,
         * starting at the given offset and for the given length.
         *
         * Fixed-keysize Tries will not support this operation (because all keys are
         * the same length).
         *
         * For example, if the trie contains 'Lime', 'LimeWire', 'LimeRadio', 'Lax',
         * 'Later', 'Lake', and 'Lovely', then a lookup of 'The Lime Plastics' with
         * an offset of 4 and a length of 4 would return 'Lime', 'LimeRadio', and
         * 'LimeWire'.
         *
         * The view that this returns is optimized to have a very efficient
         * Iterator. The firstKey, lastKey & size methods must iterate over all
         ...
         *
         * Fixed-keysize Tries can support this operation as a way to do lookups of
         * partial keys. That is, if the Trie is storing IP addresses, you can
         * lookup all addresses that begin with '192.168' by providing the key
         * '192.168.X.X' and a length of 16 would return all addresses that begin
         * with '192.168'.
         *
         * The view that this returns is optimized to have a very efficient
         * Iterator. The firstKey, lastKey & size methods must iterate over all
        ...
        /**
         * Returns a view of this map, with entries containing only those that are
         * prefixed by a value whose bits matches the bits between 'offset' and
         * 'length' in the given key.
         *
         * The view that this returns is optimized to have a very efficient
         * Iterator. The firstKey, lastKey & size methods must iterate over all
         * possible values in order to determine the results. This information is
        ...
        /**
         * Utility method for calling keyAnalyzer.bitIndex(key, 0, length(key),
         * foundKey, 0, length(foundKey))
         */
        ...
        private static class TrieEntry<K, V> implements Entry<K, V>, Serializable {
            ...
            /**
             * Whether or not the entry is storing a key. Only the root can
             * potentially be empty, all other nodes must have a key.
             */
            public boolean isEmpty() {
                return key == null;
            }
            ...
            /**
             * Replaces the old key and value with the new ones. Returns the old
             * vlaue.
             */
         ...
         * <code>KeyAnalyzer</code>'s methods return the length of the key in bits,
         ...
         * returns the bit index where one key is different from another key (if the
         * key and found key are equal than the return value is EQUAL_BIT_KEY).
         * <p>
        ...
        public static interface KeyAnalyzer<K> extends Comparator<K>, Serializable {
            ...
            /**
             * Returned by bitIndex if key and found key are equal. This is a very
             * very specific case and shouldn't happen on a regular basis
             */
            ...
            /**
             * Returns the n-th different bit between key and found. This starts the
             * comparison in key at 'keyStart' and goes for 'keyLength' bits, and
             * compares to the found key starting at 'foundStart' and going for
             * 'foundLength' bits.
             */
            ...
            /**
             * Returns the number of bits per element in the key. This is only
             * useful for variable-length keys, such as Strings.
             */
            public int bitsPerElement();

            /**
             * Determines whether or not the given prefix (from offset to length) is
             * a prefix of the given key.
             */
         ...
         * lookup if h.bitIndex > keyLength.
         */
        ...
        private class SubMap extends AbstractMap<K, V> implements SortedMap<K, V>, Serializable {
            ...
            /**
             * Constructs a blank SubMap -- this should ONLY be used by subclasses
             * that wish to lazily construct their fromKey or toKey
             */
            protected SubMap() {
            }
